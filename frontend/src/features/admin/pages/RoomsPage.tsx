...
type Room = { id: string; roomNumber: string; roomType: string; capacity: number; pricePerDay: string | number; description?: string; imageUrl?: string; status: string };
type Form = { roomNumber: string; roomType: string; capacity: string; pricePerDay: string; description: string; imageUrl: string; imageFile: File | null; status: string };
const emptyForm: Form = { roomNumber: "", roomType: "", capacity: "2", pricePerDay: "", description: "", imageUrl: "", imageFile: null, status: "AVAILABLE" };
...
export default function RoomsPage() {
  ...
  const saveMutation = useMutation({
    mutationFn: () => {
      ...
      payload.set("capacity", String(Number(form.capacity)));
      payload.set("pricePerDay", String(Number(form.pricePerDay)));
    ...
    onSuccess: async () => { await queryClient.invalidateQueries({ queryKey: ["admin", "rooms"] }); toast.success(editing ? "Room updated successfully." : "Room created successfully."); closeModal(); },
    onError: (err) => setError(getErrorMessage(err)),
  });
  const deleteMutation = useMutation({ mutationFn: (id: string) => roomApi.deleteRoom(id), onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["admin", "rooms"] }); toast.success("Room deleted successfully."); }, onError: (err) => toast.error(getErrorMessage(err)) });
  const statusMutation = useMutation({ mutationFn: ({ id, status }: { id: string; status: string }) => roomApi.updateRoom(id, { status }), onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["admin", "rooms"] }); toast.success("Room status updated."); }, onError: (err) => toast.error(getErrorMessage(err)) });
  const openCreate = () => { setEditing(null); setForm(emptyForm); setError(""); setIsOpen(true); };
  const openEdit = (room: Room) => { setEditing(room); setForm({ roomNumber: room.roomNumber, roomType: room.roomType, capacity: String(room.capacity), pricePerDay: String(room.pricePerDay), description: room.description ?? "", imageUrl: room.imageUrl ?? "", imageFile: null, status: room.status }); setError(""); setIsOpen(true); };
