    ...
    <motion.div variants={container} initial="hidden" animate="visible" className="min-h-screen bg-gray-50 p-6 dark:bg-gray-950">
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <motion.div variants={item}>
          ...
          <p className="text-sm text-gray-500">Here's what's happening at SAFNAM today.</p>
        </motion.div>
            ...
            <div className="flex h-64 items-end gap-2">
              {dashboard.revenueChart.map((point) => (
                <div key={point.label} className="flex flex-1 flex-col items-center gap-2">
                  <div className="w-full rounded-t-xl bg-emerald-500" style={{ height: `${Math.max(8, Math.min(100, Number(point.value) / 1000))}%` }} />
        ...
        <motion.section variants={item} className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-6">
            {[
              ...
              ["+ Category", "/manager/menu/categories"],
              ["+ Table", "/manager/tables"],
              ["+ Room", "/manager/rooms"],
              ["+ Offer", "/manager/offers"],
              ["Orders", "/manager/orders"],
            ].map(([label, path]) => (
              <Link key={label} to={path} className="rounded-2xl bg-emerald-50 px-4 py-4 text-sm font-semibold text-emerald-800 transition hover:scale-[1.02] dark:bg-emerald-500/10 dark:text-emerald-300">
                {label}
              </Link>
            ))}
          </div>
        ...
        <motion.div variants={item} className="grid gap-6 xl:grid-cols-2">
          ...
          <List title="Recent Feedback" items={dashboard.recentFeedback.map((feedback) => `${feedback.customerName ?? "Guest"} - Food ${feedback.foodRating ?? "-"} / Service ${feedback.serviceRating ?? "-"} - ${feedback.comments ?? ""}`)} />
        </motion.div>
...
function List({ title, items }: { title: string; items: string[] }) {
  return (
    <section className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
      ...
      <div className="space-y-3">
        {items.length ? items.map((entry) => <div key={entry} className="rounded-xl bg-gray-50 px-4 py-3 text-sm text-gray-700 dark:bg-gray-800 dark:text-gray-200">{entry}</div>) : <p className="text-sm text-gray-500">No records.</p>}
      </div>
    </section>
  );
}
